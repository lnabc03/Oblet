...
- [ ] **3.2 起始页设计**（小）
  居中排版 + 项目 logo（icons/icon-source.png，如经 1.5 提亮则用新版）+ 作者署名 **弋鹓 | lnabc03** + 操作提示（拖入文件 / Ctrl+,）。样式走主题变量。与 1.3（起始页设置不生效）联动验证。

- [ ] **3.3 检索与替换**（P0 / 中）
  自绘浮动搜索条（贴合设置浮层视觉语言），纯装饰器高亮命中（不动文档），Enter/Shift+Enter 跳转、Esc 关闭；稳定后加基础替换（逐个 + 全部，走正常事务）。
  验收：>5000 行文档搜索无卡顿；命中数显示；跳转滚动居中。

- [ ] **3.4 统一通知系统**（中）
  建立统一 toast 体系（位置、圆角、底色、滑入动画、信息/警告/错误三级）；只读横幅、外部变更提示、保存失败提示全部迁移，后续新提示一律走该系统。
