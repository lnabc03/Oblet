...
> **状态：P1 阶段全部完成（2026-08-01，一至十轮）**——批次 1-6 全项 `[x]` 闭环；唯一悬项为 4.1 Acrylic 终审判（十轮包深紫黑 alpha 250 实测，不达标则删方案）。后续打磨新项请另起清单或追加于打磨记录。
...
- [x] **3.1 底部留白**（小）✅ 2026-07-31（核对时发现前序已实现；九轮改固定值）
  ...
  十轮修订（2026-08-01）：实测（`scripts/measure-bottom-space.mjs`，真机 CDP 双尺寸）滚到底空白恒为 287px，九轮修复生效；用户所见"随窗口变大"实为短文档不滚动时的**视口内空白**（视口高 − 内容高，Obsidian/Typora 同，物理现象非 bug）。留白值设置化（界面节，0–800px 可调）供实测对比。
...
- [x] **4.1 窗口透明度定制（毛玻璃）**（中）✅ 2026-07-31
  ...
  **实机效果多轮迭代中**：Mica 按九轮批示回滚为系统材质原样；Acrylic alpha 已第四档 200，**终审判待九轮包实测——仍不达标则按预案删除 Acrylic 方案**。
  ...
  五轮修订（2026-08-01）：按批示 Acrylic tint 底色 (30,30,46) → **(24,18,42) 非常深的紫黑色**，alpha 200→**250**（98% 不透明，近实底）。终审判待十轮包实测。
...
- [x] **4.2 完善设置页面**（大）✅ 2026-07-31（随 4.1/4.4 收口）
  ...
  十轮修订（2026-08-01）：按批示大扩容 +10 项（先全量供测、按需删减）——排版：标题字体/行高/段间距/标题缩放/正文颜色/强调色（颜色 + ✕ 复位钮）；编辑器：自动保存开关/延迟（200–10000ms）/当前块开关/块底色强度档；界面：底部留白 px。输入通道归纳为 data-typo/data-num/data-color/data-check/data-select 五种通用处理器，空值写回 null。
...
- [x] **4.4 快捷键系统（可设置）**（大）✅ 2026-07-31
  ...
  十轮修订（2026-08-01）：按批示接入格式化命令——Alt+1\~6 一至六级标题（toggle 语义）、Ctrl+B 加粗、Ctrl+I 斜体、Ctrl+H 高亮、Ctrl+\` 行内代码；捕获派发 preventDefault 与 Milkdown 内置 Mod-b/i 不重复触发；新增输入守卫（焦点在输入框/设置面板时快捷键不穿透后台文档）。repro-dist/repro-webview 双环境序列化断言全过（新增 `__oblet` 验证钩子）。
...
- [x] **4.5 当前行高亮决策**（大，慎重）✅ 2026-07-31 **保留**（使用者观感评审通过）
  ...
  十轮修订（2026-08-01）：按批示调淡 alpha 0.07→**0.045**；显隐与强度设置化（编辑器节"光标所在块底色"开关 + 强度档 0.02–0.13）。
...
| zip 体积       | 4.57MB                                     | 2026-08-01 | 十轮后重封（Acrylic 深紫黑 250、快捷键扩容、设置大扩容；出口审计 MD5 MATCH + 222 许可） |
