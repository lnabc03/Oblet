...
- [x] **4.2 完善设置页面**（大）✅ 2026-07-31（随 4.1/4.4 收口）
  ...
  视觉与通知/右键菜单同语言（--ob-* 变量、8px 圆角、1px 边框、深色投影）。布局精修留待使用者观感驱动。
...
- [x] **4.5 当前行高亮决策**（大，慎重）◐ 2026-07-31 原型已上，待观感评审
  ...
  **决策点保持原样：使用者观感评审 → 保留（现状）或移除（删 main.ts 挂类 + 插件 + CSS），二选一，不留中间态。**
...
- [x] **5.2 大文档实测**（P0 / 中）✅ 2026-07-31（自动部分）
  ...
  打字跟手度/滚动帧率/自动保存耗时需使用者手感复测——若卡顿，按预案启用装饰器增量计算。
