...
> **状态：P1 阶段全部完成（2026-08-01，一至十二轮）**——批次 1-6 全项 `[x]` 闭环；Acrylic 已按十一轮终审删除（方案留档 4.1）。后续打磨新项请另起清单或追加于打磨记录。
...
- [x] **3.1 底部留白**（小）✅ 2026-07-31（核对时发现前序已实现；九轮改固定值）
  ...
  十轮修订（2026-08-01）：实测（真机 CDP 双尺寸）滚到底空白恒为 287px，九轮修复生效；用户所见"随窗口变大"实为短文档不滚动时的**视口内空白**（视口高 − 内容高，Obsidian/Typora 同，物理现象非 bug）。留白值设置化（界面节，0–800px 可调）供实测对比。
...
- [x] **3.6 右键菜单**（中）✅ 2026-07-31
  ...
  十一轮修订（2026-08-01）：按批示删除"高亮"菜单项（Ctrl+H 快捷键与工具栏按钮已覆盖该入口）。
...
- [x] **4.1 窗口透明度定制（毛玻璃）**（中）✅ 2026-07-31
  ...
  **实机效果多轮迭代中**：Mica 按九轮批示回滚为系统材质原样；Acrylic alpha 已第四档 200，**终审判待九轮包实测——仍不达标则按预案删除 Acrylic 方案**。
  ...
  六轮修订（2026-08-01，终审）：**Acrylic 方案删除**（设置下拉、commands.rs 分支、相关 CSS 全撤）。方案留档一句：window-vibrancy `apply_acrylic(tint (24,18,42,250))` 深紫黑亚克力，后续适配浅色主题时或可复用。窗口效果设置项随之收敛为 Mica 开关。
...
- [x] **4.2 完善设置页面**（大）✅ 2026-07-31（随 4.1/4.4 收口）
  ...
  十一轮修订（2026-08-01）：按批示删减定稿——排版撤全部十轮新增项（恢复原 4 项）并删提示语；编辑器自动保存延迟写死 500ms（原 1000）、块底色强度写死 0.045；界面撤底部留白、窗口效果改 Mica 开关。settings.rs 同步清字段（旧 settings.json 多出的字段 serde 读时忽略、存时丢弃）。
...
- [x] **4.4 快捷键系统（可设置）**（大）✅ 2026-07-31
  ...
  十一轮修订（2026-08-01）：新增 **Alt+A** callout 包裹（与工具栏按钮同语义：note 类型 + revertAnyType）。全部所需快捷键功能使用者实测通过。
  十二轮修订（2026-08-01）：按批示 1\~6 级标题键位写死——`Command` 加 `remappable` 标志，heading 命令设 false（设置列表不显示、keymap 覆盖忽略）。
...
- [x] **4.5 当前行高亮决策**（大，慎重）✅ 2026-07-31 **保留**（使用者观感评审通过）
  ...
  十一轮修订（2026-08-01）：0.045 使用者确认"默认刚好"**定稿**；强度档设置项按删减批示威严（写死 0.045），显隐开关保留。
...
| zip 体积       | 4.56MB                                     | 2026-08-01 | 十二轮后重封（标题键位写死、选区拖动 HTML5 DnD 落地；出口审计 MD5 MATCH + 222 许可） |
| zip 体积       | 4.56MB                                     | 2026-08-01 | 深度清理后重封（剔除夹带的本机个人设置 `data/settings.json`；审计新增"个人设置泄漏"断言，MD5 MATCH + 222 许可 + 无泄漏） |
| zip 体积       | 4.52MB                                     | 2026-08-01 | 评审修复后 pack-zip 干净暂存重封（拖动闩锁改 types 门控；审计升级硬闸门 4 项 PASS：MD5 MATCH + 210 许可 + bat 齐全 + 无 data/） |
